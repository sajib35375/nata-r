...
class ApplyFormController extends Controller
{
   ...
   public function SessionAdd(Request $request){
       ...
       if ($request->hasFile('dg_photo')){
        ...
        Asession::insert([
            ...
            'start' => $request->start,
            'end' => $request->end,
            'SL_first' => $request->first,
            'SL_last' => $request->last,
   ...
   public function SessionGet($id){
    ...
    public function updateProUpozila(Request $request,$id){
       ...
       return redirect()->route('show.upozila')->with('success','upozila updated successfully');
    ...
    public function SessionUpdate(Request $request,$id){
       $update_data = Asession::find($id);
        ...
        if ($request->hasFile('photo')){
            ...
            Image::make($img)->resize(400,417)->save('certificate/signature/'.$unique_coor);
        ...
        if ($request->hasFile('dg_photo')){
            ...
            Image::make($image)->resize(400,417)->save('certificate/signature/'.$unique_dg);
       ...
       $update_data->SL_first = $request->first;
       $update_data->SL_last = $request->last;
       $update_data->updated_at = Carbon::now();
       $update_data->update();

        return redirect()->route('show.session')->with('success','Session Updated successfully');
    }
